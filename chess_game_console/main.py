...
class Chess:
    ...
    def __init__(self, game_id=None):
        ...
        try:
            ...
            self.redis_client = None  # Si falla la conexión a Redis, continúa sin él.

        self.game_id = game_id or "default_game"  # Identificador único para la partida.

        if self.redis_client and game_id:  # Si se proporciona un game_id y hay conexión a Redis, intenta cargar la partida guardada.
    ...
    def save_game(self):
        if not self.redis_client:
            print("No hay conexión a Redis, no se puede guardar la partida.")
            return
        ...
        game_state = {
            ...
            'score': pickle.dumps(self.__score__)       # Serializa y almacena el puntaje actual.
        }
        try:
            ...
            print(f"Partida guardada correctamente con ID: {self.game_id}")
    ...
    def load_game(self):
        if not self.redis_client:
            print("No hay conexión a Redis, no se puede cargar la partida.")
            return False
        ...
        try:
            if self.redis_client.exists(self.game_id):  # Verifica si existe un estado guardado
                ...
                # Verifica si existen las claves necesarias antes de intentar cargar
                if b'board' in game_state and b'current_turn' in game_state and b'score' in game_state:
                    ...
                    self.__score__ = pickle.loads(game_state[b'score'])  # Carga el puntaje deserializado
                    print("Partida cargada correctamente.")
                    return True
                else:
                    print("Datos incompletos en la partida guardada.")
            else:
    ...
    def show_captured_pieces(self):
        ...
        print(f"Piezas capturadas blancas: {' '.join(black_captures) if black_captures else 'Ninguna'}")
        print(f"Piezas capturadas negras: {' '.join(white_captures) if white_captures else 'Ninguna'}")
