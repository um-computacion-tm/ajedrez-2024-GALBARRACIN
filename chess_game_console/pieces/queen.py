...
class Queen(Piece):
    ...
    def is_valid_move(self, start_row, start_col, end_row, end_col, board):
        ...
        # Determina el tipo de movimiento y si el camino está despejado
    ...
    def _is_clear_horizontal_path(self, start_row, start_col, end_col, board):
        ...
        return self._is_clear_linear_path(start_row, start_col + col_step, end_row=start_row, end_col=end_col, board=board, row_step=0, col_step=col_step)

    def _is_clear_vertical_path(self, start_row, end_row, start_col, board):
        row_step = 1 if end_row > start_row else -1
        return self._is_clear_linear_path(start_row + row_step, start_col, end_row, start_col, board, row_step, col_step=0)

    def _is_clear_diagonal_path(self, start_row, start_col, end_row, end_col, board):
        row_step = 1 if end_row > start_row else -1
        col_step = 1 if end_col > start_col else -1
        return self._is_clear_linear_path(start_row + row_step, start_col + col_step, end_row, end_col, board, row_step, col_step)

    # Método genérico para verificar si el camino está despejado en línea recta
    def _is_clear_linear_path(self, row, col, end_row, end_col, board, row_step, col_step):
